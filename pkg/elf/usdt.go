...
const (
	UsdtArgTypeConst UsdtArgType = iota
	UsdtArgTypeReg
	UsdtArgTypeRegDeref
)
...
func (se *SafeELFFile) UsdtTargets() ([]*UsdtTarget, error) {
	...
	if len(sections) == 0 {
		return nil, fmt.Errorf("no note section found in vDSO ELF")
	}
	...
	for _, sec := range sections {
		...
		for {
			...
			if n.DescSize < int32(unsafe.Sizeof(addrs))+3 {
				return nil, fmt.Errorf("failed to read elf note")
			}
			...
			fmt.Printf("KRAVA %s %x/%x/%x/%x %s/%s/%s\n", note,
				...
				spec.Provider, spec.Name, spec.ArgsStr)
