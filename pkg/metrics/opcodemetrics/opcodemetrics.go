...
func InitMetrics() {
	...
	for opcode := range ops.OpCodeStrings {
		if opcode != ops.MSG_OP_UNDEF && opcode != ops.MSG_OP_TEST {
			...
			LatencyStats.WithLabelValues(strconv.Itoa(int(int32(opcode))), opcode.String())
		}
	}
}
...
func GetOpTotal(opcode ops.OpCode) prometheus.Counter {
	return MsgOpsCount.WithLabelValues(strconv.Itoa(int(int32(opcode))), opcode.String())
}
